...
(function(root, factory) {
    ...
    Parser.prototype = {
        ...
        isEmptySet: function() {
            return this.firstTerm === emptySet;
        },
    ...
    Stringifier.prototype = {
         ...
         * @param {String}  [options.notation='interval'}       Use Interval.Notation.INTERVAL for '(a,e]' (default),
        ...
        stringify: function(interval, options) {
            ...
            simplify = Utils.isBoolean(opts.simplify) ? opts.simplify : true;

            if (!this.canParse(opts.notation)) {
                throw new Error('Unknown notation "' + opts.notation + '".');
            }

            if (simplify && interval.isDegenerate) {
                str = '{' + this.getDegenerateValue(interval) + '}';
            } else if (simplify && interval.isEmpty) {
                str = emptySet;
            } else if (this.isIntegerNotation(opts)) {
                str = this.toIntervalIntegerNotation(interval);
            } else {
        ...
        canParse: function(notation) {
            return typeof notation === 'undefined' || (notations.filter(function(n) {
                    return n.value === notation;
                }).length > 0);
        },
         ...
         * @param   {Number}    digits  Number of forced digits
         * @returns {Number}    value with forced decimal positions if required
         */
        getValue: function(value, digits) {
            return isNaN(digits) ? value : value.toFixed(digits);
        },

        toIntervalIntegerNotation: function(interval) {
            // Left
            var str = interval.from.value + (interval.from.included ? 0 : 1);
            // Separator
            str += '..';
            // Right
            str += interval.to.value - (interval.to.included ? 0 : 1);
            return str;
        },
        ...
        toIntervalNotation: function(interval, options) {
            ...
            var str = this.getClosure(interval.from, 'from', options.notation) +
                      this.getValue(interval.from.value, options.digits);
            // Separator
            str += ',';
            // Right
            str += this.getValue(interval.to.value, options.digits) +
    ...
    // Init notations
    Interval.registerNotations({
        notations: notations,
        parser: parser.parse.bind(parser),
        stringify: stringifier.stringify.bind(stringifier)
    });
}));